...
public class StationXMLParser extends AbstractStationParser {
	...
	public StationXMLParser(InputStream inputStream, OutputLevel level) throws IOException {
		...
		try {
			this.xmlEventReader = xmlFactory
					.createXMLEventReader(this.inputStream);
			...
			this.xmlEventFilteredReader = xmlFactory.createFilteredReader(
			...
			while ((e = xmlEventFilteredReader.peek()) != null) {
				if (e.isStartElement()) {
					...
					if (se.getName().getLocalPart()
							.equals("FDSNStationXML")) {
						root = new FDSNStationXML();
						// we only peeked, get the actual one
						se = (StartElement) xmlEventFilteredReader
								.nextEvent();
						...
						xmlEventFilteredReader.nextEvent();
						String tnos = xmlEventFilteredReader
								.getElementText();
						if (tnos != null) {
							network.setTotalNumberStations(new BigInteger(
									tnos));
						}
