...
/**
 * Class for decoding or encoding Steim2-compressed data blocks to or from an
 ...
 * <dd>Appendix B of SEED Reference Manual, 2nd Ed., pp. 119-125 <i>Federation
 * of Digital Seismic Networks, et al.</i> February, 1993</dd>
...
public class Steim2 {
	 ...
	 */
	public static DecompressedStruct decode(byte[] b, int numSamples, boolean swapBytes, int bias)
			throws SteimException {
		if (b.length % 64 != 0) {
		...
		if (debug != null) {
			debug.println("number of samples: " + numSamples + ", number of frames: " + numFrames
					+ ", byte array size: " + b.length);
		}
		for (int i = 0; i < numFrames; i++) {
			...
			for (int j = firstData; j < tempSamples.length && current < numSamples; j++) {
				...
				if (debug != null) {
					debug.print("d(" + (j - firstData) + ")" + tempSamples[j] + ", x(" + current + ")"
							+ samples[current] + ";");
				}

				if (samples[current] < min) {
					min = samples[current];
				}
				if (samples[current] > max) {
					max = samples[current];
				}
				current++;
			}
		...
		return DecompressedStruct.of(samples, min, max);
	}
	 ...
	 * @throws SteimException
	 */
	public static DecompressedStruct decode(byte[] b, int numSamples, boolean swapBytes) throws SteimException {
	...
	public static SteimFrameBlock encode(int[] samples, int frames, int bias, int samplesLength) throws SteimException {
		...
		frameBlock.addEncodedWord(samples[0], 0, 0); // X(0) -- first sample
														// value
		frameBlock.addEncodedWord(samples[samplesLength - 1], 0, 0); // X(N) --
																		// last
																		// sample
																		// value
		//
		...
		int[] minbits = new int[7]; // store min # bits required to represent
									// diff here
		...
		while (sampleIndex < samplesLength) {
			...
			// Determine the packing required for the next compressed word in
			// the SteimFrame.
	...
	private static int minBitsNeeded(int diff) {
		...
		if (diff >= -8 && diff < 8)
			...
			minbits = 6;
		else if (diff >= -128 && diff < 128)
			minbits = 8;
		else if (diff >= -512 && diff < 512)
			minbits = 10;
		else if (diff >= -16384 && diff < 16384)
			minbits = 15;
		else if (diff >= -536870912 && diff < 536870912)
			minbits = 30;
		else
			minbits = 32;
		return minbits;
	...
	private static int bitsForPack(int[] minbits, int points_remaining) {
		if (points_remaining >= 7 && (minbits[0] <= 4) && (minbits[1] <= 4) && (minbits[2] <= 4) && (minbits[3] <= 4)
				&& (minbits[4] <= 4) && (minbits[5] <= 4) && (minbits[6] <= 4))
			return 4;
		if (points_remaining >= 6 && (minbits[0] <= 5) && (minbits[1] <= 5) && (minbits[2] <= 5) && (minbits[3] <= 5)
				&& (minbits[4] <= 5) && (minbits[5] <= 5))
			return 5;
		if (points_remaining >= 5 && (minbits[0] <= 6) && (minbits[1] <= 6) && (minbits[2] <= 6) && (minbits[3] <= 6)
				&& (minbits[4] <= 6))
			return 6;
	...
	protected static int[] extractSamples(byte[] bytes, int offset, boolean swapBytes) {
		/* get nibbles */
		int nibbles = Utility.bytesToInt(bytes[offset], bytes[offset + 1], bytes[offset + 2], bytes[offset + 3],
				swapBytes);
		if (debug != null) {
			debug.printf("extractSamples: %d bytes, offset is %d, byte swap is %b, nibbles is %08X\n", bytes.length,
					offset, swapBytes, nibbles);
		}
		int currNibble = 0;
		int dnib = 0;
		...
		for (int i = 0; i < 16; i++) {
			...
			case 0:
				...
				if (offset == 0) {
					...
					temp[currNum++] = Utility.bytesToInt(bytes[offset + (i * 4)], bytes[offset + (i * 4) + 1],
							bytes[offset + (i * 4) + 2], bytes[offset + (i * 4) + 3], swapBytes);
				}
				break;
			case 1:
				if (debug != null) {
				...
				break;
			case 2:
				tempInt = Utility.bytesToInt(bytes[offset + (i * 4)], bytes[offset + (i * 4) + 1],
						bytes[offset + (i * 4) + 2], bytes[offset + (i * 4) + 3], swapBytes);
			...
			case 3:
				...
				if (diffCount > 0) {
					for (int d = 0; d < diffCount; d++) { // for-loop
															// formulation
						temp[currNum++] = (tempInt << (headerSize + (d * bitSize))) >> (((diffCount - 1) * bitSize)
								+ headerSize);
					}
				}
				break;
